...
# Age density plot broken down by lonely entrances
densityplot(~as.numeric(age), data = df, groups = compagnia,
            na.rm = TRUE, main = "Age density plot broken down by lonely entrances",
            auto.key = T, plot.point = FALSE, lwd = 3,
            par.settings = list(superpose.line = list(col = c("#4B2991","#D44292") )) )

# Age density plot broken down by churn
densityplot(~as.numeric(age), data = df, groups = si2014, plot.point = FALSE, lwd = 3,
            na.rm = TRUE, main = "Age density plot broken down by churn", auto.key = T,
            par.settings = list(superpose.line = list(col = c("#4B2991","#D44292") )) )
```
...
legend("bottomright", legend = c("Logit", "Boosted Logit", "Boosted Tree", "FDA", "Bagged Tree"),
       col = c("#F3B584", "#F66D7A", "#D44292", "#952EA0","#4B2991"), cex = .8, lty = 1:2)
```
...
```{r}
# compute profit as revenues-costs / order the column by score / cumsum of the profit

# per ogni customer:
# REV: prezzo_card base
# COST: 1$

#1)Logit
newdata1 <- test.df
newdata1$glm.prob <- glm.prob
newdata1$profit <- (newdata1$glm.prob*(28-1)) - 1
newdata1 <- newdata1[order(-newdata1$glm.prob),]
newdata1$cumprof <- cumsum(newdata1$profit)
plot(newdata1$cumprof, col = "#F3B584", ylim = c(-100,2.5e5), xlab = "nr instances",
     ylab = "Cumulative profit", main = "PROFIT CURVES for 5 models")
#2)Boosted Logit
newdata2 <- test.df
newdata2$logreg.prob <- logreg.prob[,2]
newdata2$profit <- (newdata2$logreg.prob*(28-1)) - 1
...
newdata3$profit <- (newdata3$bctree.prob*(28-1)) - 1
...
newdata4$profit <- (newdata4$fda.prob*(28-1)) - 1
